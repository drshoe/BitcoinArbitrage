...
import httplib
import json
#.market is the python file name, Market is the class name
...
class BTCChina(Market):
    ...
    def _private_request(self,post_data):
        ...
        if response.status == 200:
            ...
            print "btcchina status:",response.status
            print "btcchina reason:".response.reason
    ...
    # get current marke information    
    def update_depth(self):
        data = self.get_market_depth()
        #if result exists
    ...
    def format_depth(self, depth):
        # sort bids price as highest first
        bids = self.sort_and_format(depth['bid'], True)
        # sorts ask price as lowest first
